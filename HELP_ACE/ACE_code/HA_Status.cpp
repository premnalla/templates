...
HA_Status::init (int argc, ACE_TCHAR *argv[])
{
  ...
  ACE_DEBUG((LM_DEBUG, "Got port\n"));
  ACE_DEBUG((LM_DEBUG, "value-type=%d\n",vType));
  ACE_TString status_port;
  if (config.get_string_value (status_section,
                                ACE_TEXT ("ListenPort"),
                                status_port) == -1)
    ACE_ERROR_RETURN
      ((LM_ERROR,
